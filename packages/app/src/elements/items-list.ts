...
import "./item-icon";
import { auditVault, iconForAudit, noItemsTextForAudit, titleTextForAudit } from "../lib/audit";
                    ...
                    ${item.fields.map((f: Field, i: number) => {
                        return html`
                            <div
                                ...
                                @click=${(e: MouseEvent) => this._copyField({ item }, i, e)}
                                draggable="true"
                                @dragstart=${(e: DragEvent) => this._dragFieldStart({ item }, i, e)}
                            >
...
export class ItemsList extends StateMixin(LitElement) {
    ...
    render() {
        const { favorites, attachments, host, tag, report: audit } = this.filter || {};
        ...
        const heading = favorites
            ...
            : vault
            ? { title: vault.name, superTitle: "", icon: "vaults" }
            : tag
    ...
    private _getItems(): ListItem[] {
        const { tag, favorites, attachments, host, report: audit } = this.filter || {};
        ...
        if (filter) {
            items = vault
                ? [...vault.items].filter((item) => filterByString(filter || "", item)).map((item) => ({ item }))
                : [];
        } else if (host) {
