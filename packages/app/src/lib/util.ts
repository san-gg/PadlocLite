...
export async function handleSave(name: string, type: string, contents: Uint8Array) {
    const dialog = getSingleton("pl-dialog") as Dialog<null, null>;
    dialog.show();
    dialog.loading = true;
    try {
        await saveFile(name, type, contents);
        await alert("File saved to Downloads", {
            title: "Done",
            type: "success",
            zIndex: 11,
        });
        ...
        if (
            e.code == ErrorCode.INSUFFICIENT_PERMISSIONS ||
            e.code == ErrorCode.SOMETHIND_WENT_WRONG ||
            e.code == ErrorCode.FILE_ALREADY_PRESENT
        ) {
