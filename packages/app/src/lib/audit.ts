...
export async function auditVault(updateOnlyIfOutdated: boolean = false) {
    const reusedPasswords: ListItem[] = [];
    const weakPasswords: ListItem[] = [];
    const compromisedPasswords: ListItem[] = [];
    const expiredItems: ListItem[] = [];
    const vault: Vault | null = app.mainVault;

    if (vault == null) return { reusedPasswords, weakPasswords, compromisedPasswords, expiredItems };
    ...
    try {
                ...
                if (app.account?.settings.securityReport.compromisedPaswords) {
                    ...
                    if (isPasswordCompromised) {
                        ...
                        if (!compromisedPasswordItemIds.has(item.id)) {
                            compromisedPasswords.push({ item });
        ...
        vaultResultsFound = false;
        reusedPasswords.length = 0;
        weakPasswords.length = 0;
        compromisedPasswords.length = 0;
        expiredItems.length = 0;
    }
    if (vaultResultsFound) await app.saveVault(vault);
